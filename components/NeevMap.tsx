...
export const NeevMap: React.FC<any> = ({
  ...
  const handleZoneClick = (zone: LibraryZone) => {
          ...
          <div className="flex items-center justify-between">
            ...
            {locatedShelfId && (
              <button
                ...
                className="text-[10px] font-mono px-2 py-0.5 bg-muted border border-border text-blue-400 hover:text-foreground rounded transition animate-pulse"
              >
        ...
        {selectedZone ? (
          <div className="bg-muted/50/30 border border-border/60 rounded-2xl p-6 h-full flex flex-col justify-between space-y-6">
            <div className="space-y-4">
              ...
              <div>
                <span className="text-[10px] uppercase font-mono bg-primary/10 border border-blue-900/40 text-blue-400 px-2 py-0.5 rounded-md">
                ...
                <h3 className="text-base font-bold text-foreground mt-2.5 tracking-tight border-b border-border/60/80 pb-2 flex items-center justify-between">
                  ...
                  <span className={`w-2.5 h-2.5 rounded-full ${
                    selectedZone.status === 'busy' ? 'bg-amber-500' : selectedZone.status === 'alert' ? 'bg-red-500' : 'bg-green-500'
              ...
              {selectedZone.type === 'shelf' ? (
                <div className="space-y-3">
                  <div className="flex items-center space-x-2">
                    <Layers className="w-3.5 h-3.5 text-blue-500 hover:rotate-18 hover:text-foreground transition" />
                    ...
                    {activeShelfBooks.map((book) => (
                      ...
                      >
                        <div className="min-w-0 pr-2">
                          <h5 className="text-xs font-bold text-foreground truncate group-hover:text-blue-400 transition-colors">{book.title}</h5>
                ...
                <div className="space-y-3 pt-2">
                  ...
                  <div className="bg-background p-3 rounded-xl border border-border space-y-2 font-mono text-[10px] text-muted-foreground">
                    <div className="flex justify-between">
                      <span className="text-zinc-600">Hardware UUID:</span>
                      <span>neev-{selectedZone.id}-77AX</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-zinc-600">NFE Frequency:</span>
                      <span>865.6 MHz - 867.6 MHz</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-zinc-600">Gateways Protocol:</span>
                      <span>EPC Class 1 Gen 2 / ISO 18000-6C</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-zinc-600">NFC Deck Range:</span>
