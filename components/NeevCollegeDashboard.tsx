...
export const NeevCollegeDashboard: React.FC<any> = ({
  ...
  const handleApproveHandoffTransfer = async () => {
    try {
      setActionErrorMsg('');
      const res = await onKioskCheckout(studentSelectId, selectedPhysicalCopyId, checkoutType, stampUrl || "https://images.unsplash.com/photo-1544947950-fa07a98d237f?auto=format&fit=crop&q=80&w=200");
  ...
  const handleCompleteReturn = async (e: React.FormEvent) => {
    ...
    try {
      setReturnStatusMsg('');
      setActionErrorMsg('');
      const res = await onKioskReturn(returnPhysicalCopyId, returnCondition);
  ...
  const handleCompleteBountyIntake = async (e: React.FormEvent) => {
    ...
    try {
      setBountyStatusMsg('');
      setActionErrorMsg('');
      const res = await onBountyIntake(bountyIsbn, donorStudentId);
  ...
  const handleTriggerWebhook = async () => {
    try {
      setActionErrorMsg('');
      await onSimWebhook(webhookType, webhookAmount, webhookStudentId);
      addXp(80);
    } catch (err: any) {
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-muted/50/40 border border-slate-200/60 rounded-2xl p-6 flex items-center justify-between">
          ...
          <div className="w-12 h-12 rounded-xl bg-emerald-950/45 border border-emerald-900/40 flex items-center justify-center text-emerald-400 shrink-0">
        ...
        <div className="bg-muted/50/40 border border-slate-200/60 rounded-2xl p-6 flex items-center justify-between">
          ...
          <div className="w-12 h-12 rounded-xl bg-blue-950/45 border border-primary/50 flex items-center justify-center text-blue-400 shrink-0">
      ...
      <div className="bg-muted/50/10 border border-slate-200 rounded-2xl p-6">
        ...
        {deskTab === 'checkout' && (
          <div className="space-y-6">
            <div className="flex items-center justify-between border-b border-slate-200 pb-3">
              ...
              <span className="text-[10px] font-mono px-2 py-0.5 bg-indigo-950 text-primary rounded-md border border-indigo-900 font-bold uppercase shrink-0">
              ...
              <form onSubmit={handleStartCheckout} className="space-y-4 text-xs font-sans">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="flex items-end justify-end">
                    <button
                      type="submit"
                      className="w-full sm:w-auto px-6 py-3 bg-indigo-600 hover:bg-indigo-500 text-xs font-bold text-foreground rounded-xl transition flex items-center justify-center space-x-2"
                    >
            ...
            {handoffStep === 'scanned_qr' && (
              <div className="space-y-4 animate-in fade-in">
                ...
                <div className="p-4 bg-amber-950/15 border border-amber-900/40 text-amber-500 text-[11px] rounded-xl leading-relaxed flex items-start space-x-2.5">
            ...
            {handoffStep === 'capturing_spine' && (
              <div className="p-4 bg-background border border-slate-200 rounded-xl space-y-4 animate-in zoom-in-95">
                ...
                <div className="flex justify-end pt-1">
                  <button onClick={handleSimulateCaptureFinished} className="px-5 py-2 bg-emerald-600 hover:bg-emerald-550 text-foreground font-bold rounded-lg text-xs flex items-center space-x-1.5">
            ...
            {handoffStep === 'transferred' && (
              <div className="p-6 bg-emerald-950/10 border border-emerald-900/30 rounded-xl text-center space-y-3 animate-in zoom-in-95">
                <div className="w-10 h-10 rounded-full bg-emerald-950 border border-emerald-850 flex items-center justify-center text-emerald-400 mx-auto animate-bounce">
        ...
        {deskTab === 'return' && (
          <form onSubmit={handleCompleteReturn} className="space-y-6 text-xs text-left">
            ...
            {returnStatusMsg && (
              <div className="p-3 bg-emerald-950/30 border border-emerald-900/50 rounded-xl text-emerald-400 font-mono text-[11px] font-semibold">
                {returnStatusMsg}
              </div>
            )}
            ...
            <div className="flex justify-end pt-1">
              <button
                type="submit"
                className="w-full sm:w-auto px-6 py-3 bg-indigo-600 hover:bg-indigo-500 text-foreground font-bold rounded-xl flex items-center justify-center space-x-1.5"
              >
                <UserCheck className="w-4 h-4" />
        ...
        {deskTab === 'bounty' && (
          <div className="space-y-6 text-left text-xs">
            <div className="border-b border-slate-200 pb-3 flex justify-between items-center">
              ...
              <span className="text-[9px] font-mono px-2 py-0.5 bg-emerald-950 text-emerald-400 border border-emerald-900 rounded font-bold uppercase">
                Active Bounties: {bountyBoard.length}
              </span>
            </div>

            {bountyStatusMsg && (
              <div className="p-3 bg-emerald-900/10 border border-emerald-800/40 rounded-xl text-emerald-400 font-semibold mb-3">
                {bountyStatusMsg}
              </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
              ...
              <form onSubmit={handleCompleteBountyIntake} className="md:col-span-1 space-y-4 bg-background p-5 rounded-2xl border border-slate-200">
                ...
                <button
                  type="submit"
                  className="w-full py-2 bg-emerald-600 hover:bg-emerald-550 text-foreground font-bold rounded-xl tracking-wider uppercase transition text-[10.5px] mt-1"
                >
        ...
        {deskTab === 'finance' && (
          <div className="space-y-6 text-left text-xs">
            <div className="border-b border-slate-200 pb-3 flex justify-between items-center">
              ...
              <span className="text-[9px] font-mono px-2 py-0.5 bg-blue-950 border border-blue-900 text-blue-300 rounded font-bold uppercase">
                    ...
                    {financialLogs?.map((log, idx) => (
                      <div key={idx} className="px-4 py-2.5 grid grid-cols-12 gap-2 text-muted-foreground hover:bg-muted/50/40">
                        ...
                        <span className="col-span-4 flex items-center space-x-1.5 font-bold">
                          <span className={`w-1.5 h-1.5 rounded-full ${log.category === 'OPERATIONAL_REVENUE_ANNUAL' ? 'bg-indigo-400' : 'bg-emerald-400'}`} />
                        ...
                        <span className={`col-span-2 text-right font-extrabold pb-0.5 ${log.category === 'OPERATIONAL_REVENUE_ANNUAL' ? 'text-foreground/90' : 'text-emerald-400'}`}>
        ...
        {deskTab === 'telemetry' && (
          <div className="space-y-6 text-left text-xs">
            <div className="border-b border-slate-200 pb-3 flex justify-between items-center">
              ...
              <span className="text-[9px] font-mono px-2 py-0.5 bg-red-950 border border-red-900 text-red-400 rounded font-bold uppercase">
                  ...
                  {violations?.map((v, idx) => (
                    <div key={idx} className="p-3.5 space-y-1.5 text-muted-foreground hover:bg-muted/50/30">
                      <div className="flex justify-between items-center">
                        <span className="text-red-400 font-extrabold flex items-center space-x-1">
                      ...
                      <div className="flex justify-between text-[9px] text-muted-foreground/60">
                        ...
                        <span className="bg-red-950/40 text-red-400 font-bold border border-red-900/20 px-1 rounded uppercase">Logged to registry</span>
                      </div>
                    </div>
                  ))}
