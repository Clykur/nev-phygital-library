  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-[20px] duration-500">
      ...
      <div className="lg:col-span-1 space-y-6">
        ...
        <div className="relative aspect-[1.586/1] w-full rounded-2xl bg-gradient-to-tr from-indigo-600 via-indigo-900 to-slate-900 border border-indigo-400/40 p-6 shadow-2xl flex flex-col justify-between overflow-hidden group">
          ...
          <div className="flex items-start justify-between">
            <div className="space-y-0.5">
              <span className="text-[9px] font-mono tracking-widest text-indigo-200 uppercase font-semibold">Neev Phygital System</span>
            ...
            <div className="w-8 h-8 rounded-full bg-white/5 border border-white/10 flex items-center justify-center">
              <svg viewBox="0 0 24 24" className="w-4 h-4 text-indigo-200 fill-none stroke-current" strokeWidth="2" strokeLinecap="round">
          ...
          <div className="flex items-end justify-between border-t border-white/10 pt-4 z-10">
            ...
            <span className="text-[9px] font-mono text-indigo-200 uppercase">Level {currentLevel} Member</span>
          </div>
        ...
        <div className={`bg-zinc-90 w-full border rounded-2xl p-5 space-y-4 transition-all duration-500 relative overflow-hidden ${
          flashCreditFull 
            ? 'border-emerald-500 ring-2 ring-emerald-500/20 shadow-[0_0_25px_rgba(16,185,129,0.2)] bg-emerald-950/10' 
            : 'border-border/60 bg-muted/50/40'
        }`}>
          {flashCreditFull && (
            <div className="absolute top-2 right-2 px-2 py-0.5 bg-emerald-950 border border-emerald-500/30 text-emerald-400 text-[9px] font-bold font-mono rounded animate-pulse">
              ✔ RECONCILED OK
            </div>
          )}
          ...
          <div className="w-full h-3 bg-background border border-border rounded-lg overflow-hidden flex relative">
            ...
            <div 
              className={`h-full bg-gradient-to-r from-indigo-500 via-blue-500 to-indigo-600 transition-all duration-700 ${
                flashCreditFull ? 'animate-pulse bg-gradient-to-r from-emerald-500 to-emerald-400' : ''
              }`} 
          ...
          <div className="flex justify-between items-center text-[10px] font-mono">
            ...
            <div className="flex items-center space-x-1">
              <span className={`w-2 h-2 rounded block ${flashCreditFull ? 'bg-emerald-500 animate-ping' : 'bg-indigo-500'}`}></span>
              <span className={`${flashCreditFull ? 'text-emerald-400 font-bold' : 'text-primary'} uppercase transition-all`}>
      ...
      <div className="lg:col-span-2 space-y-6">
                  ...
                  <div className="space-y-1 tracking-tight min-w-0 flex-1">
                    ...
                    <div className="pt-2 w-full max-w-sm space-y-1">
                      <div className="w-full h-1 bg-muted/50 rounded-full overflow-hidden">
                        <div 
                          className={`h-full rounded-full transition-all ${
                            checkout.daysRemaining < 3 ? 'bg-red-500' : checkout.daysRemaining < 7 ? 'bg-amber-500' : 'bg-emerald-500'
                          }`}
                          style={{ width: `${rRatio}%` }}
                        ></div>
                      </div>
                      <div className="flex justify-between text-[9px] font-mono text-muted-foreground/60">
                        ...
                        <span className={checkout.daysRemaining < 3 ? 'text-red-400 font-bold' : ''}>{checkout.daysRemaining} days remaining till {checkout.dueDate}</span>
                      </div>
                    </div>
                  </div>
        ...
        <div className={`border rounded-2xl p-6 space-y-4 transition-all duration-500 relative overflow-hidden ${
          geofenceBreached 
            ? 'bg-red-950/15 border-red-500 ring-2 ring-red-500/20 shadow-[0_0_20px_rgba(239,68,68,0.15)]' 
          ...
          <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-3 border-b border-border pb-3">
            <div className="flex items-center space-x-2">
              <Map className={`w-4 h-4 ${geofenceBreached ? 'text-red-400 animate-pulse' : 'text-blue-400'}`} />
              <div>
            ...
            <div className="md:col-span-2 flex justify-center">
              <div className="relative w-28 h-28 rounded-full border border-border flex items-center justify-center bg-black/40 overflow-hidden shrink-0">
                ...
                <div className={`absolute select-none font-mono text-[7px] font-bold tracking-widest bottom-2 text-center w-full transition ${
                  geofenceBreached ? 'text-red-500 animate-pulse' : 'text-emerald-500 opacity-60'
                }`}>
                ...
                <div 
                  className={`absolute w-3 h-3 rounded-full border border-white transition-all duration-700 ${
                    geofenceBreached 
                      ? 'left-[82%] top-[15%] bg-red-500 shadow-[0_0_8px_#ef4444]' 
                      : 'left-[46%] top-[46%] bg-blue-500 shadow-[0_0_8px_#3b82f6]'
                  }`}
                >
              ...
              <div className="flex items-center space-x-2 pt-1">
                {geofenceBreached ? (
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 bg-red-950/20 hover:bg-red-950/40 border border-red-900/40 text-red-400 font-bold font-mono text-[10px] uppercase rounded-lg transition flex items-center space-x-1"
                  >
          ...
          {geofenceBreached && (
            <div className="p-3 bg-red-950/20 border border-red-900/60 rounded-xl flex items-start space-x-2 text-xs text-red-400 font-light leading-relaxed animate-bounce">
              ...
              return (
                ...
                >
                  <div className={`p-2 rounded-lg border flex-shrink-0 ${
                    ach.unlocked ? 'bg-amber-950/40 border-amber-800/60 text-amber-400 animate-[bounce_3s_ease_infinite]' : 'bg-muted/50 border-border/60 text-zinc-650'
                  }`}>
                  ...
                  <div className="space-y-1 min-w-0 text-left">
                    ...
                    {ach.unlocked && (
                      <span className="text-[9px] font-mono text-emerald-400 font-bold">&#10004; Reward claimed: {ach.xp} XP</span>
                    )}
                  </div>
                </div>
              );
