    ...
    const closed =
      status === "expired"
        ...
        : status === "cancelled"
          ? "Withdrawn — closed"
          : "Closed";
    return <p className="caption-scale font-medium text-foreground-muted">{closed}</p>;
  }
  return (
    <div
    ...
    >
      {DESK_PROGRESS_LABELS.map((label, i) => (
        <span key={label} className="flex items-center">
          {i > 0 ? (
            <span className="mx-0.5 caption-scale text-foreground-subtle" aria-hidden>
              →
            </span>
          ) : null}
          <span
            role="listitem"
            className={cn(
              "inline-flex h-6 items-center rounded-md border px-2 caption-scale font-medium",
              i === active
                ? "border-primary/30 bg-primary/10 text-foreground"
                : i < active
                  ? "border-secondary/30 bg-secondary/10 text-secondary/80"
                  : "border-border/70 bg-background text-foreground-muted",
            )}
          >
            {i < active ? "✓ " : ""}
            {label}
  ...
  if (!user?.hubStaffHubIds.length) {
    return (
      <div className={cn("mx-auto max-w-lg pb-20 text-center", PORTAL_PAGE_GUTTER_X, inShell ? "pt-8" : "pt-28")}>
        ...
        <p className="mt-2 text-sm text-foreground-muted">Hub staff memberships unlock this queue.</p>
    ...
    <div className={cn(topPad, "pb-10 lg:pb-10")}>
      <div className="mb-6 border-b border-border pb-5">
        <div className="min-w-0 font-sans">
          <h1 className={cn("mt-1", PORTAL_PAGE_TITLE)}>Requests</h1>
          ...
          {user.hubStaffHubIds.length > 1 ? (
            <div className="flex min-w-0 flex-1 flex-col gap-1.5">
              ...
              <Select value={requestsHubId} onValueChange={setRequestsHubId}>
                <SelectTrigger id="hub-req-scope" className={cn(adminSelectTrigger, "text-primary")}>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
      ...
      <section className={cn(adminPanel, "overflow-hidden")} aria-label="Request queue">
                      ...
                      <div className="min-w-0 flex-1 space-y-3 font-sans">
                        ...
                        {r.createdAt ? (
                          <p className="caption-scale font-medium text-foreground-muted">
                            Open{" "}
                            <time dateTime={r.createdAt} className="text-foreground">
                              {formatDistanceToNow(new Date(r.createdAt), { addSuffix: true })}
                            </time>
                          </p>
                        ) : null}
                        {r.notes?.trim() ? (
                          <p className="rounded-md border border-border/70 bg-muted/30 px-3 py-2 body-scale text-foreground-muted">
                            {r.notes}
                          </p>
                        ) : null}
                        <div className="grid gap-2 rounded-md border border-border bg-muted/[0.2] px-3 py-2.5 caption-scale font-medium sm:grid-cols-2 lg:grid-cols-4">
                          <div className="space-y-0.5">
                            <p className="text-foreground-muted">Member</p>
                            <p className="font-mono text-foreground">{r.requesterPublicId ?? "Student"}</p>
                          </div>
                          <div className="space-y-0.5">
                            <p className="text-foreground-muted">Hub</p>
                            <p className="truncate text-foreground">{hubName(r.hubId)}</p>
                          </div>
                          <div className="space-y-0.5">
                            <p className="text-foreground-muted">Updated</p>
                            <p className="text-foreground">{fmtDeskReqDate(r.updatedAt ?? r.createdAt)}</p>
                          </div>
                          <div className="space-y-0.5">
                            <p className="text-foreground-muted">Ready at</p>
                            <p className="text-foreground">{r.readyAt ? fmtDeskReqDate(r.readyAt) : "—"}</p>
                          </div>
                        </div>
                        <p className="rounded-md border border-border/70 bg-background/70 px-3 py-2 caption-scale font-medium text-foreground-muted">
                          <span className="font-semibold text-foreground">Assigned copy: </span>
                          {r.assignedCopyId ? (
                            <>
                              <span className="font-mono text-foreground">{r.assignedCopyRefId ?? "Linked copy"}</span>
                        ...
                        {r.assignedCopyId && r.assignmentVerified === false ? (
                          <p className="rounded-md border border-primary/30 bg-primary/10 px-3 py-2 caption-scale font-medium text-foreground">
                            Not shelf verified - pickup may fail.
                          </p>
                        ) : null}
                        {(ACTIVE_PIPELINE as readonly string[]).includes(r.status) && r.bookTitle?.trim() ? (
                          <p className="rounded-md border border-border/70 bg-background/70 px-3 py-2 caption-scale font-medium text-foreground-muted">
                            <span className="font-semibold text-foreground">Available matches on shelf: </span>
                        ...
                        {nextAction ? (
                          <div className="rounded-md border border-primary/25 bg-primary/[0.06] px-3 py-2 body-scale font-normal leading-snug text-foreground bg-primary/10">
                            <span className="font-semibold text-primary">Next: </span>
                            {nextAction}
                          </div>
                        ) : null}
                        {hint ? (
                          <p className="body-scale font-normal leading-relaxed text-foreground-muted">{hint}</p>
                        ) : null}
                      </div>
                      {showActionColumn ? (
                        <div className="flex flex-col items-stretch gap-2 lg:min-w-[14rem] lg:max-w-[14rem]">
                          {r.status === "requested" ? (
                            <Button
                              ...
                              size="sm"
                              variant="secondary"
                              className="h-9 rounded-md body-scale font-medium"
                          ...
                          {r.status === "fulfilled" ? (
                            <Button
                              type="button"
                              size="sm"
                              className="h-9 rounded-md body-scale font-normal"
                              disabled={pendingPatch}
                          ...
                          {!r.assignedCopyId &&
                            ...
                            !!r.bookTitle?.trim() &&
                            matchAvailableForRequest(r) === 0 ? (
                            <p className="rounded-md border border-border/70 bg-background/70 px-2.5 py-2 caption-scale font-medium text-foreground-muted">
                            ...
                            <>
                              {r.assignmentVerified === false ? (
                                <Button
                                  ...
                                  size="sm"
                                  variant="outline"
                                  className="h-8 rounded-md body-scale font-normal"
                            ...
                            <div className="mt-1 rounded-md border border-border bg-muted/[0.2] p-2.5">
                              ...
                              <div className="mt-2 flex flex-col gap-1.5">
                                <Button
                                  ...
                                  size="sm"
                                  variant="outline"
                                  className="h-8 w-full justify-start body-scale font-normal"
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className="font-sans h4-scale font-semibold text-foreground">Admin-close this request?</AlertDialogTitle>
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="font-sans h4-scale font-semibold text-foreground">Reassign to another hub</DialogTitle>
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="font-sans h4-scale font-semibold text-foreground">Override request status</DialogTitle>
            <DialogDescription className="body-scale text-foreground-muted">
